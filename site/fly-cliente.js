...
window.Corpo3D=(function(){
  ...
  function desenharMacho(q, tt){
    if(S.raiz<0) return;
    const X=S.sexo; const est=X.estado; S.macho.visible=S.espelhoM.visible=(est!=='idle'); let alvo=POSES[est]||POSES.idle; if(est==='mating') alvo={p:alvo.p, yaw:alvo.yaw, pitch:S.aj.pitch}; const u=Math.min(1,(performance.now()-X.t_est)/700);
    ...
    if(est==='mating'){ const ph=(tt*ritmo)%1; const f=ph<0.3?Math.sin(ph/0.3*Math.PI/2):Math.cos((ph-0.3)/0.7*Math.PI/2); const amp=0.10*(0.4+0.6*lib); px+=amp*f; pz+=0.03*f; pitch+=-0.06*f; X.empurrao=amp*f*0.5; X.rabo=S.aj.raboBase+(S.aj.raboAmp+0.2*lib)*f; roll=0.03*Math.sin(tt*2*Math.PI*ritmo*0.5);
      ...
      for(const perna of ['LF','LM','LH','RF','RM','RH']){ const fe=S.asas['joint_'+perna+'Femur'], ti=S.asas['joint_'+perna+'Tibia'], cx=S.asas['joint_'+perna+'Coxa']; const tipo=perna[1]; if(tipo==='F'){ if(cx!=null) qM[cx]-=0.35; if(fe!=null) qM[fe]-=0.6; if(ti!=null) qM[ti]+=1.0; } else if(tipo==='M'){ if(fe!=null) qM[fe]-=0.45; if(ti!=null) qM[ti]+=0.9; } else { if(fe!=null) qM[fe]-=0.2; if(ti!=null) qM[ti]+=0.9; } }   // frente: agarra o abdomen dela; meio: apoia; tras: esticadas para o chao
