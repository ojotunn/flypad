...
ARQ_CONFIG = DADOS / 'estado' / QUEM / 'config.json'   # so 'ela' grava/serve config, mercado e corpo     # CA do token e link do X (publicos), sobrevivem a reinicio
...
async def api_dormir(request):
    """FLY PAD: o gerente poe a mosca para dormir. Salva as sinapses e encerra o processo (a placa fica livre)."""
    cerebro = request.app['cerebro']
    try:
        cerebro.parar()
    except Exception as e:
        print(f'[servidor] erro ao salvar antes de dormir: {e}', flush=True)
    print('[servidor] dormindo (sinapses salvas)', flush=True)
...
def main():
    ...
    app.router.add_post('/api/dormir', api_dormir)
