...
SERVIDOR_ELE = os.environ.get('FLY_SERVIDOR_ELE', '')   # FLY PAD: vazio = mosca sozinha (so um cerebro)   # o cerebro do macho: sente o mercado igual
...
PASTA = Path(os.environ.get('FLY_MERCADO_DIR') or Path(__file__).resolve().parent)   # FLY PAD: uma pasta por mosca
IGNORAR_ARQ = PASTA / 'ignorar.txt'   # tokens que ela NUNCA opera (o dela): um endereco por linha
...
SENTIDOS_ARQ = PASTA / 'sentidos.txt'   # token cujos trades sao os SENTIDOS dela (o dela)
...
ORDENS_ARQ = PASTA / 'ordens.txt'     # 'on' | 'off': liga/desliga as ordens sem reiniciar
ULTIMO_ARQ = PASTA / 'ultimo_token.txt'   # token que ela opera (para retomar apos reinicio)
AJUSTES_ARQ = PASTA / 'ajustes.txt'       # max_ordem_usd=10  lote=0.10  (relido ao vivo)
...
def estimular(nome, ms, quem='ambos'):
    ...
    ok = False
    for alvo in ((['ela', 'ele'] if SERVIDOR_ELE else ['ela']) if quem == 'ambos' else [quem]):
        if alvo == 'ele' and not SERVIDOR_ELE:
            continue
        try:
